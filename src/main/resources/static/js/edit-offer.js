(function ($) {
    const URLS = {
        attrs: '/api/offer/attributes',
        update: '/api/offer/edit',
    };
    ...
    $(() => {
        ...
        categorySelect.val(categorySelect.attr('data-value'));

        loadCategoryInputs();
        fetchAttributes(id)
            .then(response => {
                container.find('.input-attr').each(function () {
                    const input = $(this);
